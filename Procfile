...
release: sh -c 'cd decide && python manage.py makemigrations && python manage.py migrate && python manage.py loaddata data.json && echo "from django.contrib.auth import get_user_model; User = get_user_model();\nif not User.objects.filter(email='"'"'admin@decide.com'"'"').exists(): User.objects.create_superuser('"'"'admin@decide.com'"'"', '"'"'practica'"'"')" | python3 ./manage.py shell'
